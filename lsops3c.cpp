 ...
 * lsops3c.cpp
 *
 * This file holds functions to manipulate the signed distance 
 * function in order to update the location of the zero level set.
 * 
...
void ls_iteration(double *F, double *phi, double* label, long* dims, 
  ...
  for(i=0;i<Lz->length;i++) F[i] = F[i]/Fmax*0.49;
  ...
  while(Lz->curr != NULL){
    ...
    if(phi_old<=0 && phi[idx]>0 ){
      //mexPrintf("in2out\n");
      ll_pushnew(Lin2out,x,y,z);
    }
    if(phi_old>0  && phi[idx]<=0){
      //mexPrintf("out2in\n");
      ll_pushnew(Lout2in,x,y,z);
    }
  ...
  ll_init(Ln1);
  while(Ln1->curr != NULL){
    x = Ln1->curr->x; y = Ln1->curr->y; z = Ln1->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
    pmax = ls_max_hood_onlevel(idx, x, y, z, dims, phi,label,0);
    if(pmax>=-.5){
      phi[idx] = pmax-1;
  ...
  ll_init(Lp1);
  while(Lp1->curr != NULL){
    x = Lp1->curr->x; y = Lp1->curr->y; z = Lp1->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
    pmin = ls_min_hood_onlevel(idx, x, y, z, dims, phi,label,0);
    if(pmin<=.5){
      phi_old = phi[idx];
      phi[idx] = pmin+1;
  ...
  ll_init(Ln2);
  while(Ln2->curr != NULL){
    x = Ln2->curr->x; y = Ln2->curr->y; z = Ln2->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
    pmax = ls_max_hood_onlevel(idx, x, y, z, dims, phi,label,-1);
    if(pmax>=-1.5){
      phi[idx] = pmax-1;
      if(phi[idx]>=-1.5){
  ...
  ll_init(Lp2);
  while(Lp2->curr != NULL){
    x = Lp2->curr->x; y = Lp2->curr->y; z = Lp2->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
    pmin = ls_min_hood_onlevel(idx, x, y, z, dims, phi,label,1);
    if(pmin<1.5){
      phi[idx] = pmin+1;
      if(phi[idx]<=1.5){
  ...
  ll_init(Sz);
  while(Sz->curr != NULL){
    x = Sz->curr->x; y = Sz->curr->y; z = Sz->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
  ...
  ll_init(Sn1);
  while(Sn1->curr != NULL){
    x = Sn1->curr->x; y = Sn1->curr->y; z = Sn1->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
    ll_push(Ln1,ll_remcurr(Sn1));
    label[idx] = -1;
    if(((y+1)<DIMY) && phi[idx+OFFY]==-3){
      ll_pushnew(Sn2,x,y+1,z); 
  ...
  ll_init(Sp1);
  while(Sp1->curr != NULL){
    x = Sp1->curr->x; y = Sp1->curr->y; z = Sp1->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
    ll_push(Lp1,ll_remcurr(Sp1));
    label[idx] = 1;
    if(((y+1)<DIMY) && phi[idx+OFFY]==3){
      ll_pushnew(Sp2,x,y+1,z); 
      phi[idx+OFFY] = phi[idx] + 1;
    }//up
    if(((y-1)>=0)   && phi[idx-OFFY]==3){
      ll_pushnew(Sp2,x,y-1,z); 
  ...
  ll_init(Sn2);
  while(Sn2->curr != NULL){
    x = Sn2->curr->x; y = Sn2->curr->y; z = Sn2->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
  ...
  ll_init(Sp2);
  while(Sp2->curr != NULL){
    x = Sp2->curr->x; y = Sp2->curr->y; z = Sp2->curr->z;    
    idx = (int)(z*DIMXY+x*DIMY+y);
  ...
  while(Lz->curr != NULL){
    ...
    if(((y+1)<DIMY) && abs((int)label[idx+OFFY])==3){//up
      if(phi[idx+OFFY]<0){//in
        ...
        ll_pushnew(Ln1,x,y+1,z);
      }
      else{                //out
        label[idx+OFFY]=1; phi[idx+OFFY]=1;
        ll_pushnew(Lp1,x,y+1,z);
      }
    }
    ...
    if(((x+1)<DIMX) && abs((int)label[idx+OFFX])==3){//right
      if(phi[idx+OFFX]<0){//in
        ...
        ll_pushnew(Ln1,x+1,y,z);
      }
      else{                //out
        label[idx+OFFX]=1; phi[idx+OFFX]=1;
        ll_pushnew(Lp1,x+1,y,z);
      }
    }
    ...
    if(((z+1)<DIMZ) && abs((int)label[idx+OFFZ])==3){//front
      if(phi[idx+OFFZ]<0){//in
        ...
        ll_pushnew(Ln1,x,y,z+1);
      }
      else{                //out
        label[idx+OFFZ]=1; phi[idx+OFFZ]=1;
        ll_pushnew(Lp1,x,y,z+1);
      }
    }
...
//find the max of nearby points on a specified level set
